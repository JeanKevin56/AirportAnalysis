...
class FlowDashboard:
    ...
    Affiche l'historique glissant des 10 dernières minutes de comptage,
    les prédictions à court terme et la répartition par caméra.
    """
    ...
    def setup_plot(self):
        ...
        self.fig.suptitle('AeroFlow - Dashboard de suivi des flux (Historique 10 min)', 
    ...
    def update(self, history: List[int], prediction: Optional[float], 
        ...
        Args:
            history: Historique des comptages (fenêtre glissante de 10 minutes)
            prediction: Prédiction à 30 secondes
            current_a: Comptage actuel caméra A
            current_b: Comptage actuel caméra B
            trend: Tendance (HAUSSE, BAISSE, STABLE)
        """
        ...
        try:
            ...
            # Graphique 1: Historique sur 10 minutes et prédiction
            if len(history) > 0:
                x = list(range(len(history)))


                self.ax1.plot(x, history, 'b-', linewidth=3, label='Flux total observé')
                ...
                if prediction is not None and len(history) >= 3:
                    ...
                    self.ax1.plot([len(history)-1, pred_x], [history[-1], prediction], 
                               'r--', linewidth=3, label=f'Prédiction +{config.PREDICTION_HORIZON}s',
                               alpha=0.8)
            ...
            self.ax1.set_xlabel('Mesures (intervalle: {}s) - Historique 10 minutes'.format(config.SAMPLING_INTERVAL), 
                              fontsize=12, fontweight='bold')
            self.ax1.set_ylabel('Nombre de personnes', fontsize=12, fontweight='bold')
            self.ax1.set_title('Evolution du flux et prédiction (fenêtre glissante 10 min)', 
                             fontsize=13, pad=10)
            ...
            # Graphique 2: Répartition caméras (état actuel)
            cameras = ['Caméra A', 'Caméra B', 'Total']
            ...
            self.ax2.set_title('Répartition actuelle par caméra', fontsize=13, pad=10)
    ...
    def _format_info_panel(self, current_a, current_b, trend, prediction, history):
        ...
        Affiche les statistiques actuelles et les métriques calculées
        sur la fenêtre glissante de 10 minutes.
        """
        total = current_a + current_b


        info = f"STATISTIQUES (10 min)\n"
        info += f"{'-'*30}\n"
        ...
        if prediction is not None:
            info += f"Préd 30s: {prediction:3.0f}\n"
...
class FrameAnnotator:
    ...
    def annotate_frame(frame: np.ndarray, count: int, camera_name: str, 
        ...
        # Texte principal
        text1 = f"Caméra {camera_name} ({mode})"
        ...
        # Prédiction
        if prediction is not None and mode == "Maître":
            text3 = f"Prédiction 30s: {prediction:.0f}"
